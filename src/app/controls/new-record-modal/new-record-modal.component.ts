...
export class NewRecordModalComponent implements OnInit {
  ...
  weekDates: string[] = []; // Array to hold formatted dates (e.g., "Sun 03/23")
  ...
  constructor(
    ...
    // Set default week start and create the form group with day-specific fields
    this.selectedWeekStart = this.getCurrentWeekStart();
  ...
  // Auto-populate country and project code based on selected project
  ...
  // CSS class for date highlighting in the datepicker
  ...
  // Handle date selection from the date range input
  ...
  // Set the week dates (for display and saving)
  private setWeekDates(startOfWeek: Date): void {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      this.weekDates.push(this.formatDate(date));  // Store formatted label (e.g., "Sun 03/23")
    }
  }

  // Format date as "Day MM/DD"
  ...
  // Helper methods to return form control names for hours and remarks per day
  getDayControl(index: number): string {
    return ['sun', 'mon', 'tue', 'wed', 'thu', 'fri', 'sat'][index];
  }
  getRemarkControl(index: number): string {
    return ['sunRemark', 'monRemark', 'tueRemark', 'wedRemark', 'thuRemark', 'friRemark', 'satRemark'][index];
  }
    ...
    if (this.timesheetForm.valid) {
      ...
      forkJoin(saveRequests).subscribe({
        next: () => {
          ...
          // Reset day-specific controls
          this.timesheetForm.patchValue({
            sun: 0, mon: 0, tue: 0, wed: 0, thu: 0, fri: 0, sat: 0,
            sunRemark: '', monRemark: '', tueRemark: '', wedRemark: '', thuRemark: '', friRemark: '', satRemark: ''
          });
        },
        error: (err) => {
  ...
  getDayPairs(): number[][] {
    const pairs: number[][] = [];
    for (let i = 0; i < this.weekDates.length; i += 2) {
      if (i + 1 < this.weekDates.length) {
        pairs.push([i, i + 1]); // Pairing two days together
      } else {
        pairs.push([i]); // In case of an odd number of days, last one remains alone
      }
    }
    return pairs;
  }
  
  // Create records for each day that has hours entered
  private createRecordsFromForm(formValue: any): TimesheetRecord[] {
    const dayKeys = ['sun', 'mon', 'tue', 'wed', 'thu', 'fri', 'sat'];
    const remarkKeys = ['sunRemark', 'monRemark', 'tueRemark', 'wedRemark', 'thuRemark', 'friRemark', 'satRemark'];
    const baseRecord = {
    ...
    dayKeys.forEach((day, index) => {
      ...
      if (hours > 0) {
        const remark = formValue[remarkKeys[index]];
        const date = this.weekDateObjects[index];
        const year = date.getFullYear();
        const month = (date.getMonth() + 1).toString().padStart(2, '0');
        const dayOfMonth = date.getDate().toString().padStart(2, '0');
        const dateWorked = `${year}-${month}-${dayOfMonth}`; // Format: YYYY-MM-DD
        records.push({
          ...baseRecord,
          hoursWorked: hours,
  ...
  // Get the current week's Sunday as default (formatted)
